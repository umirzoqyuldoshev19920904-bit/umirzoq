...
# Sahifa konfiguratsiyasini sozlash (Keng ekran rejimi)
st.set_page_config(
    page_title="UltraWarehouse Pro",
    page_icon="📦",
    layout="wide"
)

# HTML, CSS va JS interfeys kodlari
html_code = """
<!DOCTYPE html>
<html lang="uz">
<head>
        ...
        <main class="flex-1 flex flex-col h-screen overflow-y-auto">
            <header class="bg-white border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sticky top-0 z-40 shadow-sm">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Tizim holati: <span class="text-emerald-500 font-medium">Onlayn</span></p>
                </div>
                ...
                <div x-show="activeTab === 'products'" x-cloak class="space-y-4">
                    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="relative">
                            ...
                            <input type="text" x-model="searchQuery" placeholder="Nom yoki SKU..." class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none">
                        </div>
                        <div>
                            <select x-model="filterWarehouse" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none">
                        ...
                        <div>
                            <select x-model="filterStock" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none">
                                <option value="all">Barcha Holatlar</option>
                                <option value="low">Kam qolganlar (<10)</option>
                                <option value="good">Yetarli miqdor</option>
                            </select>
